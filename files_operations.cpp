 ...
 */
int SaveToFile(std::string message, int lines_treshold,
...
bool IsPhoto(std::string &file_name) {
	...
	std::string file_extension = file_name.substr(
			file_name.find_last_of(".") + 1, file_name.length());
	file_name = file_name.substr(file_name.find_last_of("/") + 1,
			file_name.length());
	if (file_extension == "bmp" || file_extension == "jpg"
			|| file_extension == "png" || file_extension == "jpeg")
		return true;
	else
		return false;
}
...
void set_config(Configuration &config) {
	...
	tab_of_lines[0] = tab_of_lines[0].substr(
			tab_of_lines[0].find(delimiter) + 1, tab_of_lines[0].length());
	tab_of_lines[1] = tab_of_lines[1].substr(
			tab_of_lines[1].find(delimiter) + 1, tab_of_lines[1].length());
